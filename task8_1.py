...
class Date:
    ...
    @staticmethod
    def check_date(date):
        day, month, year = Date.extract(date)  # использование classmethod
        good_date = True

        def bad_date():
            """Присваивает переменной good_date значение False"""
            nonlocal good_date
            good_date = False
        ...
        if month == 2 and is_leap_year(year) and (day > 29 or day < 1):  # проверка даты в феврале високосного года
            ...
            print(f"Для месяца {month} введено не верное количество дней {day}")
            bad_date()
        if year > 2050 or year < 0:  # проверка года от 0 до 2050
...
Date.check_date("30-12-2038")
print()
Date.check_date("30-02-2001")
print()
Date.check_date("30-09-2049")
print()
Date.check_date("29-08-1990")
print()
Date.check_date("29-02-1907")